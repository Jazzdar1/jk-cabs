...
const { Pool } = require('pg'); // Naya Cloud DB Manager
...
// GET APIs
app.get('/api/fleet', async (req, res) => { const result = await pool.query(`SELECT * FROM fleet`); res.json(result.rows); });
app.get('/api/drivers', async (req, res) => { const result = await pool.query(`SELECT * FROM drivers`); res.json(result.rows); });
app.get('/api/admin/bookings', async (req, res) => { const result = await pool.query(`SELECT * FROM bookings ORDER BY id DESC`); res.json(result.rows); });
app.get('/api/customer/bookings/:email', async (req, res) => { const result = await pool.query(`SELECT * FROM bookings WHERE customer_email = $1 ORDER BY id DESC`, [req.params.email]); res.json(result.rows); });
